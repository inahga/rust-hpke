...
fn client_encrypt_msg(
    ...
    let (encapped_key, mut sender_ctx) =
        hpke_pq::setup_sender::<Aead, Kdf, Kem, _>(&OpModeS::Base, server_pk, INFO_STR, &mut csprng)
...
fn server_decrypt_msg(
    ...
    let mut receiver_ctx =
        hpke_pq::setup_receiver::<Aead, Kdf, Kem>(&OpModeR::Base, &server_sk, &encapped_key, INFO_STR)
